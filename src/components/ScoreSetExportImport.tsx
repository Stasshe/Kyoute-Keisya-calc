...
type Props = {
  scoreSets: ScoreSet[];
  onImport: (scoreSets: ScoreSet[]) => void;
};

export default function ScoreSetExportImport({ scoreSets, onImport }: Props) {
  const [showImport, setShowImport] = useState(false);
  const [importText, setImportText] = useState('');

  const handleExport = async () => {
    const exportData = JSON.stringify(scoreSets, null, 2);
    ...
    try {
      ...
      for (const item of parsed) {
        if (
          ...
          validScoreSets.push(item as ScoreSet);
        }
      }

      if (validScoreSets.length === 0) {
        alert('有効な保存データが見つかりませんでした。');
        return;
      }

      onImport(validScoreSets);
      setImportText('');
      setShowImport(false);
  ...
  return (
    <div className="bg-white rounded-lg border shadow-sm p-3 space-y-2">
      ...
      <div className="text-xs text-gray-500 pt-2">
        <p>※ エクスポート：全ての保存データをクリップボードにコピーします</p>
