...
export default function ResultsTab({ scores, department, universityName, facultyName }: Props) {
  ...
  const weightTotal = useMemo(() => {
    return subjectDetails.filter(s => !s.isIgnored).reduce((sum, s) => sum + (s.weight || 0), 0);
  }, [subjectDetails]);

  // 各教科の配点率データとドメイン自動計算（最大値を切り上げて見やすくする）
  const weightRateData = useMemo(() => {
    const data = subjectDetails
      .filter(s => !s.isIgnored)
      .map(s => ({
        subject: s.label,
        配点率: weightTotal > 0 ? Math.round((s.weight / weightTotal) * 1000) / 10 : 0,
      }));

    const maxRate = data.length > 0 ? Math.max(...data.map(d => d.配点率)) : 0;
    const roundedMax = Math.max(10, Math.ceil(maxRate / 10) * 10);

    return { data, maxRate, domainMax: roundedMax };
  }, [subjectDetails, weightTotal]);
  ...
  return (
    <div className="p-3 space-y-4 pb-6">
      ...
      <div className="bg-white rounded-lg border shadow-sm overflow-hidden">
        <div className="bg-gradient-to-r from-gray-50 to-gray-100 px-3 py-2 border-b">
          <h3 className="font-semibold text-gray-900 text-sm">レーダーチャート分析</h3>
        </div>
        <div className="p-3">
            ...
            <div className="w-full h-64">
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart data={weightRateData.data}>
                  ...
                  <PolarRadiusAxis
                    angle={90}
                    domain={[0, weightRateData.domainMax]}
                    tick={{ fontSize: 10 }}
                  />
                  <Radar
                    name="配点率 (%)"
                  ...
                  <Tooltip
                    ...
                    formatter={(value: number) => [`${Math.round(value * 10) / 10}%`, '配点率']}
                  />
                  <Legend wrapperStyle={{ fontSize: '12px' }} />
                </RadarChart>
              </ResponsiveContainer>
