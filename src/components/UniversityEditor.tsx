...
export default function UniversityEditor({ initialUniversity, onSave, onCancel }: Props) {
  const [univ, setUniv] = useState<University>({ ...initialUniversity, faculties: initialUniversity.faculties.map(f=>({ ...f, departments: f.departments.map(d=>({ ...d })) })) });
  ...
  function updateFacultyName(facId: string, name: string) {
    setUniv(u=> ({ ...u, faculties: u.faculties.map(f=> f.id===facId ? { ...f, name } : f) }));
  }

  function addFaculty() {
    const id = `fac_${Date.now()}`;
    const newF: Faculty = { id, name: '新しい学部', departments: [{ id: `${id}_d1`, name: '新しい学科', weights: SUBJECTS.reduce((acc,s)=>({ ...acc, [s.key]: 0 }), {} as Weights) }] };
  ...
  function updateDepartmentName(facId: string, deptId: string, name: string) {
    setUniv(u=> ({ ...u, faculties: u.faculties.map(f=> f.id===facId ? { ...f, departments: f.departments.map(d=> d.id===deptId ? { ...d, name } : d) } : f) }));
  }

  function updateDeptWeight(facId: string, deptId: string, key: string, value: string) {
    const n = parseFloat(value);
    setUniv(u=> ({ ...u, faculties: u.faculties.map(f=> f.id===facId ? { ...f, departments: f.departments.map(d=> d.id===deptId ? { ...d, weights: { ...d.weights, [key]: Number.isFinite(n) ? n : 0 } } : d) } : f) }));
  }
  ...
  return (
    <div className="p-4 bg-card border rounded-md shadow-sm">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {SUBJECTS.map(s => (
                      <div key={s.key}>
                        ...
                        <input value={String(d.weights[s.key] ?? 0)} onChange={(e)=> updateDeptWeight(f.id, d.id, s.key, e.target.value)} className="mt-1 w-full px-2 py-1 border rounded-md text-sm" />
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <div className="mt-3 flex gap-2 justify-end">
        <button onClick={onCancel} className="px-3 py-2 border rounded-md text-sm">キャンセル</button>
        <button onClick={()=> onSave(univ)} className="px-3 py-2 bg-primary text-primary-foreground rounded-md text-sm">保存</button>
        <button onClick={addFaculty} className="px-3 py-2 border rounded-md text-sm">学部追加</button>
      </div>
    </div>
  );
}
